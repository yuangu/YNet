...
void YDNSResolver::request(std::string& host, YDNSType type = DNSType_A, const char* DNSServer = "114.114.114.114")
{
	...
	client.onRead([&](YUDPSocket*, YAddress*, const char* buff, int len) {
		this->onRespond(buff);
	});
}

void YDNSResolver::onRespond(const char* _buff)
{
	...
	char* tmp = (char*)_buff + 12;

	//请求区域
	for (int i = 0; i < ntohs(header->q_count); ++i)
	{
		int readLeng = 0;
		std::string name = readName((char*)_buff, tmp - _buff, readLeng);
		tmp += readLeng;
