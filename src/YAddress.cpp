...
void YAddress::reset(std::string& host, unsigned short port)
{
	...
	else if (is_ipv6_address(host))
	{
		_isIPV6 = true;
	}
	else if (is_valid_domain_name(host))
	{
		YDNSResolver resolver;
		std::vector<std::string> ret;
		if(resolver.lookupByName(host, ret)|| ret.size() <= 0) return;
		tmp.clear();
		tmp.append(ret[0]);
	}
	else {
	...
	if (_isIPV6)
	{
		...
		inet_pton(AF_INET6, tmp.c_str(), &(addr_v6->sin6_addr));
	}
	else {
		struct sockaddr_in *addr_v4 = (struct sockaddr_in *)&mss;
		addr_v4->sin_family = AF_INET;
		addr_v4->sin_port = htons(port);
		inet_pton(AF_INET, tmp.c_str(), &(addr_v4->sin_addr));
	}
	resetHostAndPort();
}
